...
func InitShortSnowflake(workerID, datacenterID int64) (*ShortSnowflake, error) {
	if workerID > maxShortWorkerID || workerID < 0 {
		return nil, errors.New("worker ID must be between 0 and 3")
	}
	if datacenterID > maxShortDatacenterID || datacenterID < 0 {
		return nil, errors.New("datacenter ID must be between 0 and 3")
	}
