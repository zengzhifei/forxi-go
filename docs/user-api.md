...
| verification\_code | string | 是  | 6位验证码 |
...
| 参数名         | 类型     | 说明              |
| ----------- | ------ | --------------- |
| bind\_token | string | 已登录用户绑定OAuth时使用 |
...
| 参数名               | 类型     | 必填 | 说明          |
| ----------------- | ------ | -- | ----------- |
| bind\_token       | string | 是  | 绑定令牌（从回调获取） |
| email             | string | 是  | 要绑定的邮箱地址    |
| email\_code       | string | 是  | 邮箱验证码       |
...
**描述**: 通过URL在线预览文件，服务器会下载文件并上传到存储，返回文件URL。
...
```json
{
  ...
  "message": "success",
  "data": {
    "url": "https://cdn.forxi.cn/filereview/xxx.jpg",
    "name": "文件名",
    "type": "image|video|audio|pdf|office|text"
  }
}
```
